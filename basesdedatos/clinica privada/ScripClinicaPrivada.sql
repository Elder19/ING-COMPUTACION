...
begin
    create table dbo.Resultado_Cita (
        IdCita int not null, -- Agregar IdCita para referencia
        resultado nvarchar(200) not null,
        foreign key (IdCita) references Cita(IdCita)
    )
end;

-- Crear la tabla 'Tratamientos_Cita' si no existe
...
-- Crear la tabla 'Medicamento_Cita' si no existe
...
begin
    create table dbo.Medicamento_Cita (
        IdCita int not null, -- Agregar IdCita para referencia a Cita
        medicamentos nvarchar(100) not null,
        duracion time not null,
        primary key (IdCita), -- Usar IdCita como parte de la clave primaria
        foreign key (IdCita) references Cita(IdCita),
...
begin
    create table dbo.Procedimiento (
        IdProcedimiento int identity(1,1) not null, -- Campo ID autoincrementable
        N_carnet nvarchar(100) not null, -- Cambiado a N_carnet para referenciar adecuadamente
        N_cedula nvarchar(100) not null, -- Cambiado a N_cedula para referenciar adecuadamente
        fecha date not null,
        Duracion time not null,
        epicrisis nvarchar(200) not null,
        primary key (IdProcedimiento), -- Usar IdProcedimiento como clave primaria
...
begin
    create table dbo.Tratamiento_Procedimiento (
        IdTratamiento int identity(1,1) not null, -- Campo ID autoincrementable
        IdProcedimiento int not null, -- Referencia al procedimiento
        tratamiento nvarchar(100) not null,
        primary key (IdTratamiento), -- Usar IdTratamiento como clave primaria
...
begin
    create table dbo.Medicamento_Procedimiento (
        IdMedicamento int identity(1,1) not null, -- Campo ID autoincrementable
        IdProcedimiento int not null, -- Referencia al procedimiento
        medicamentos nvarchar(100) not null,
        duracion time not null,
        primary key (IdMedicamento), -- Usar IdMedicamento como clave primaria
        foreign key (IdProcedimiento) references Procedimiento(IdProcedimiento),
        foreign key (medicamentos) references Medicamentos(nombre)
    )
end;
...
VALUES 
(1, 'Paracetamol', '02:00:00'), -- Duración de 2 horas
(2, 'Ibuprofeno', '03:00:00'),  -- Duración de 3 horas
(3, 'Amoxicilina', '01:30:00'), -- Duración de 1 hora y 30 minutos
(4, 'Aspirina', '02:30:00');     -- Duración de 2 horas y 30 minutos
...
VALUES 
(1, 'Paracetamol', '01:00:00'), -- Duración de 1 hora
(2, 'Ibuprofeno', '02:00:00'), -- Duración de 2 horas
(3, 'Amoxicilina', '00:30:00'), -- Duración de 30 minutos
(4, 'Aspirina', '01:15:00'); -- Duración de 1 hora y 15 minutos
